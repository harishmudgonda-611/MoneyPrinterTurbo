...
def test_reel_task_runner_records_render_qa():
    ...
    ), patch("app.services.reel_task_runner.sm.state.patch_task", return_value=True) as patch_task:
    ...
    assert patch_task.call_args.kwargs["state"] == const.TASK_STATE_COMPLETE
...
def test_reel_task_runner_converts_render_qa_failure_to_failed_task():
    ...
    patch_task.assert_called_once()
    assert patch_task.call_args.kwargs["state"] == const.TASK_STATE_FAILED
